-- Count pixels within a 2-minute range based on a reference pixel's timestamp
-- 
...
--   3. Count all pixels within a 2-minute range (±1 minute) of that timestamp
--
-- Example: Find pixel at (100, 200) and count all pixels within 2 minutes of its timestamp

-- Option 1: Using updated_at timestamp (modify x and y values)
DO $$
DECLARE
    target_x INTEGER :=91;  -- Replace with your x coordinate
    target_y INTEGER := 421;  -- Replace with your y coordinate
...
BEGIN
    ...
    -- Count pixels within the time range
    SELECT COUNT(*) INTO pixel_count
    FROM pixels
    WHERE updated_at >= start_time
      AND updated_at <= end_time;
    
    RAISE NOTICE 'Reference pixel at (%, %) had timestamp: %', target_x, target_y, reference_timestamp;
    RAISE NOTICE 'Found % pixels between % and %', pixel_count, start_time, end_time;
END $$;
