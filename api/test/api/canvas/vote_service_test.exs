defmodule Api.Canvas.VoteServiceTest do
  ...
  describe "settle_daily_votes/0" do
    ...
    test "returns no_votes_to_settle when no unsettled votes exist" do
      assert {:ok, :no_votes_to_settle} = VoteService.settle_daily_votes()
    end
    ...
    test "marks votes as settled", %{voter: voter, target_plot: target_plot} do
      ...
      {:ok, %{total_votes: 1}} = VoteService.settle_daily_votes()
    ...
    test "creates log for voter with votes_cast", %{voter: voter, target_plot: target_plot} do
      ...
      {:ok, _} = VoteService.settle_daily_votes()
    ...
    test "handles multiple votes on the same plot", %{
      ...
      {:ok, _voter2_plot} = create_plot(voter2.id, "Voter2 Plot")
      ...
      {:ok, %{total_votes: 2, processed_users: 3}} = VoteService.settle_daily_votes()
    ...
    test "plot owner receives 100 pixels per vote from multiple voters", %{
      ...
      # Create more voters
      {:ok, voter2} = create_user("voter2@example.com")
      {:ok, _} = create_plot(voter2.id, "Voter2 Plot")
    ...
    test "settles votes for specific date", %{voter: voter, target_plot: target_plot} do
      {:ok, _} = VoteService.cast_vote(voter.id, target_plot.id)

      today = Date.utc_today()
      {:ok, %{date: ^today}} = VoteService.settle_daily_votes(today)

      # Vote should be settled
      settled_vote = Vote.Repo.get_user_plot_votes(voter.id, target_plot.id) |> List.first()
      assert settled_vote.settlement_date == today
    end

    test "re-aggregation updates existing log instead of creating duplicate", %{
      voter: voter,
      plot_owner: plot_owner,
      target_plot: target_plot
    } do
      ...
      # First settlement
      {:ok, _} = VoteService.settle_daily_votes(today)

      owner_logs_after_first =
        Log.Repo.list_by_user(plot_owner.id)
        |> Enum.filter(&(&1.log_type == "daily_vote_aggregate"))

      assert length(owner_logs_after_first) == 1
      first_log = List.first(owner_logs_after_first)
      assert first_log.metadata["settledAt"] == Date.to_iso8601(today)
