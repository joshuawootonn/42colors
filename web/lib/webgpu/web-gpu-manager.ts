...
export class WebGPUManager {
  ...
  async initialize(): Promise<void> {
    this.polygonRenderer = await createWebGPUPolygonRenderer(this.device, this.canvasFormat);
    this.pixelRenderer = await createWebGPUPixelRenderer(this.device, this.canvasFormat);
    this.lineRenderer = await createWebGPULineRenderer(this.device, this.canvasFormat);
  ...
  redrawPolygons(items: PolygonRenderItem[], options: Partial<PolygonRenderOptions> = {}): void {
    if (!this.polygonRenderer) {
      throw new Error("WebGPU polygon renderer not initialized. Call initialize() first.");
    }
  ...
  redrawLines(lines: LineRenderItem[], options: Partial<LineRenderOptions> = {}): void {
    if (!this.lineRenderer) {
      throw new Error("WebGPU line renderer not initialized. Call initialize() first.");
    }
   ...
   */
  redrawPixels(pixels: Pixel[], options: Partial<PixelRenderOptions> = {}): void {
    if (!this.pixelRenderer) {
      throw new Error("WebGPU pixel renderer not initialized. Call initialize() first.");
    }
  ...
  if (navigator.gpu) {
    try {
      ...
      if (webgpuContext) {
        ...
        const manager = new WebGPUManager(device, webgpuContext, canvasFormat);
