...
export class WebGPUBufferPool {
    ...
    async processFrameCompletion(): Promise<void> {
        ...
        const toReturn = this.pendingReturns.filter(
            (item) => item.frameId <= safeFrameId,
        );
    ...
    private cleanup(): void {
        ...
        if (activeBuffers.length > this.maxPoolSize) {
            ...
            const toRemove = unusedBuffers.slice(
                0,
                activeBuffers.length - this.maxPoolSize,
            );
    ...
    getStats(): {
        ...
        const memoryInUse = this.buffers
            .filter((pb) => pb.inUse)
            .reduce((sum, pb) => sum + pb.size, 0);
