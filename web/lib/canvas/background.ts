import { BACKGROUND_SCALE, BACKGROUND_SIZE, CANVAS_PIXEL_RATIO } from "../constants";
...
export function drawBackgroundCanvas(canvas: HTMLCanvasElement, context: CanvasRenderingContext2D) {
  ...
  for (let x = startPoint.x; x <= endPoint.x; x += CANVAS_PIXEL_RATIO * BACKGROUND_SCALE) {
    context.moveTo(x, startPoint.y);
    context.lineTo(x, endPoint.y);
  }

  for (let y = startPoint.y; y <= endPoint.y; y += CANVAS_PIXEL_RATIO * BACKGROUND_SCALE) {
