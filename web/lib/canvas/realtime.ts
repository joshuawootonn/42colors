...
export function resizeRealtimeCanvas(canvas: HTMLCanvasElement, camera: Camera) {
...
function getActionsHash(actions: unknown[]): number {
  ...
  if (actions.length > 0) {
    ...
    const pointsLength = Array.isArray(lastAction?.points) ? lastAction.points.length : 0;
