...
export const newPixelResponseSchema = z.object({
  pixels: z.array(z.object({ x: z.number(), y: z.number(), color_ref: colorRefSchema })),
  store_id: z.string(),
});
...
export function newPixels(context: InitializedStore, pixels: Pixel[], action_id?: string) {
...
export function setupChannel(socket: Socket): Channel {
  ...
  channel.on("create_plot", (payload: { plot: Plot; chunk_keys: string[] }) => {
    ...
    chunk_keys.forEach((chunkKey) => {
      ...
      const localPlot: Plot = {
        ...
        polygon: polygonSchema.parse({
          vertices: plot.polygon.vertices.map((vertex) => [vertex[0] - chunkX, vertex[1] - chunkY]),
        }),
      };
