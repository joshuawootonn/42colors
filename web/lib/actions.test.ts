...
describe("actions", () => {
  test("brushing and undoing unsets pixels", () => {
    ...
    expect(deriveUnsetPixelsFromActions(arr)).toEqual(pointsToPixels(points4to6, 1));
  ...
  describe("updateActionBasedOnRejectedPixels", () => {
    test("removes rejected pixels from brush action", () => {
      ...
      const rejectedPixels: Pixel[] = [{ x: 1, y: 1, color_ref: 1 } as Pixel];
      ...
      if (updatedAction.type !== "brush-active") throw new Error("Wrong type");
    ...
    test("removes rejected pixels from erasure action", () => {
      ...
      const updatedAction = updated[0];
      if (updatedAction.type !== "erasure-active") throw new Error("Wrong type");
      expect(updatedAction.points).toHaveLength(2);
      expect(updatedAction.points).toEqual([
        { x: 2, y: 2 },
        { x: 3, y: 3 },
    ...
    test("removes rejected pixels from line action", () => {
      ...
      const updatedAction = updated[0];
      if (updatedAction.type !== "line-complete") throw new Error("Wrong type");
      expect(updatedAction.points).toHaveLength(2);
      expect(updatedAction.points).toEqual([
        { x: 2, y: 2 },
        { x: 3, y: 3 },
    ...
    test("removes rejected pixels from bucket action", () => {
      ...
      const otherAction = Actions.bucket(
        [absolutePointTupleSchema.parse([4, 4]), absolutePointTupleSchema.parse([5, 5])],
        2,
      );
      const rejectedPixels: Pixel[] = [{ x: 1, y: 1, color_ref: 1 } as Pixel];
      ...
      const updatedAction = updated[0];
      if (updatedAction.type !== "bucket-active") throw new Error("Wrong type");
      expect(updatedAction.points).toHaveLength(2);
      expect(updatedAction.points).toEqual([
        [2, 2],
        [3, 3],
    ...
    test("ignores actions with different action_id", () => {
      ...
      const updated = updateActionBasedOnRejectedPixels([action], rejectedPixels, "different-id");
