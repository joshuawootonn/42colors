...
const API_URL = process.env.NEXT_PUBLIC_API_ORIGIN ?? "https://api.42colors.com";
...
const registerResponseSchema = z.union([successResponseSchema, errorResponseSchema]);

const loginResponseSchema = z.union([successResponseSchema, errorResponseSchema]);
...
const updatePasswordResponseSchema = z.union([successResponseSchema, errorResponseSchema]);
...
const authService = {
  ...
  async getCurrentUser(origin: string): Promise<UserResponse | null> {
    try {
      ...
      if (!parseResult.success) {
        console.error("Invalid user response format:", parseResult.error);
        return null;
      }
