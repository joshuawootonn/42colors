...
import { ClaimerSettings, claimerSettingsSchema } from "./tools/claimer/claimer-settings";
...
export function updateToolSettings(toolSettings: ToolSettings) {
  ...
  window.localStorage.setItem(createToolKey(ERASURE_SIZE), toolSettings.erasure.size.toString());
  window.localStorage.setItem(createToolKey(LINE_SIZE), toolSettings.line.size.toString());
...
const stringToToolSchema = z.string().transform((val) => toolSchema.safeParse(val).data);
...
export function getToolSettings(): ToolSettings | undefined {
  ...
  const paletteIsOpen = stringToBooleanSchema.safeParse(getByKey(PALETTE_IS_OPEN));
  ...
  const toolSettings: ToolSettings = {
    ...
    palette: {
      ...
      foregroundColorRef:
        paletteForegroundColorRef.data ?? DEFAULT_TOOL_SETTINGS.palette.foregroundColorRef,
      backgroundColorRef:
        paletteBackgroundColorRef.data ?? DEFAULT_TOOL_SETTINGS.palette.backgroundColorRef,
    },
    claimer: {
      selectedPlotId: undefined,
    },
