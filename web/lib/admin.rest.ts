...
const grantPixelsResponseSchema = z.union([
    grantPixelsSuccessSchema,
    grantPixelsErrorSchema,
]);

export type GrantPixelsResponse = z.infer<typeof grantPixelsResponseSchema>;

export async function grantPixels(
    userId: number,
    amount: number,
): Promise<GrantPixelsResponse> {
    ...
    const response = await fetch(
        new URL(`/api/admin/grant_pixels`, context.server.apiOrigin),
        {
            method: 'POST',
            credentials: 'include',
            headers: {
    ...
    if (!parseResult.success) {
        console.error(
            'Invalid grant pixels response format:',
            parseResult.error,
        );
