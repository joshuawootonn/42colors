...
function redrawTelegraph(context: InitializedStore) {
  ...
  if (!pixelTelegraphWebGPUManager) {
    console.error("Telegraph WebGPU manager not available for line telegraph rendering");
    return;
  }
...
export function nextLineAction(activeLineAction: LineActive, endPoint: AbsolutePoint): LineActive {
...
function onPointerDown(
  ...
  const nextActiveAction = startLineAction(startPoint, color_ref, context.toolSettings.line.size);
