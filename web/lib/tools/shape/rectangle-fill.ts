...
import {
  completeRectangleFillAction,
  nextRectangleFillAction,
  startRectangleFillAction,
} from "./shape-helpers";
...
function redrawTelegraph(context: InitializedStore) {
  ...
  if (!pixelTelegraphWebGPUManager) {
    console.error("Telegraph WebGPU manager not available for rectangle-fill telegraph rendering");
    return;
  }

  const pixelSize = getPixelSize(getZoomMultiplier(context.camera));

  if (context.activeAction && context.activeAction.type === ACTION_TYPES.RECTANGLE_FILL_ACTIVE) {
...
function onPointerDown(
  ...
  const nextActiveAction = startRectangleFillAction(startPoint, color_ref);
...
function onPointerMove(
  ...
  if (context.activeAction?.type !== ACTION_TYPES.RECTANGLE_FILL_ACTIVE) {
    return context;
  }
  ...
  const nextActiveAction = nextRectangleFillAction(
...
function onPointerOut(
  ...
  const rectangleComplete = completeRectangleFillAction(context.activeAction);
...
function onPointerUp(
  ...
  const action_id = context.activeAction.action_id;
  const completedAction = completeRectangleFillAction(context.activeAction);
...
export const RectangleFillTool = {
  onPointerMove,
  onPointerDown,
  onPointerUp,
  onPointerOut,
...
export type RectangleFillTool = typeof RectangleFillTool;
