...
import { completeEllipseAction, nextEllipseAction, startEllipseAction } from "./shape-helpers";
...
function redrawTelegraph(context: InitializedStore) {
  ...
  if (!pixelTelegraphWebGPUManager) {
    console.error("Telegraph WebGPU manager not available for ellipse telegraph rendering");
    return;
  }

  const pixelSize = getPixelSize(getZoomMultiplier(context.camera));

  if (context.activeAction && context.activeAction.type === ACTION_TYPES.ELLIPSE_ACTIVE) {
...
function onPointerDown(
  ...
  const nextActiveAction = startEllipseAction(
    startPoint,
    color_ref,
...
function onPointerMove(
  ...
  if (context.activeAction?.type !== ACTION_TYPES.ELLIPSE_ACTIVE) {
    return context;
  }
  ...
  const nextActiveAction = nextEllipseAction(
...
function onPointerOut(
  ...
  const ellipseComplete = completeEllipseAction(context.activeAction);
...
function onPointerUp(
  ...
  const action_id = context.activeAction.action_id;
  const completedAction = completeEllipseAction(context.activeAction);
...
export type EllipseTool = typeof EllipseTool;
