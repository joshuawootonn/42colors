...
export function pointsToPixels(points: AbsolutePoint[], color_ref: ColorRef): Pixel[] {
  return points.map(
    (point) =>
      ({
        x: point.x,
...
export function isDuplicatePoint(canvasX: number, canvasY: number, context: InitializedStore) {
...
function onPointerDown(
  ...
  const brushPoints = getBrushPoints([anchorPoint], context.toolSettings.brush.size, 1);
  ...
  const nextActiveAction = startBrushAction(anchorPoint, brushPoints, color_ref);
...
function onPointerMove(
  ...
  if (context.activeAction?.type !== ACTION_TYPES.BRUSH_ACTIVE || isDuplicatePoint(x, y, context)) {
    return context;
  }
  ...
  const newBrushPoints = getBrushPoints(netNewAnchors, context.toolSettings.brush.size, 1);

  const nextActiveAction = nextBrushAction(context.activeAction, newAnchorPoints, newBrushPoints);
...
function onPointerOut(
  ...
  if (context.activeAction?.type !== ACTION_TYPES.BRUSH_ACTIVE) return context;
