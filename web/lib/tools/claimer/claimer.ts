...
function redrawTelegraph(context: InitializedStore) {
  ...
  if (!uiTelegraphWebGPUManager) {
    console.error("Telegraph WebGPU manager not available for claimer telegraph rendering");
    return context;
  }
  ...
  if (
    ...
    if (context.activeAction.nextRect != null) {
      actionPolygons.push(rectToPolygonSchema.parse(context.activeAction.nextRect));
    }
...
export function startNewRectCreateAction(curr: ClaimerCreate, rect: Rect): ClaimerNewRectCreate {
...
export function completeNewRectCreateAction(curr: ClaimerNewRectCreate): ClaimerCreate {
  return {
...
export function completeRectangleClaimerAction(polygons: Polygon[]): ClaimerComplete {
  return {
...
export function startNewRectEditAction(curr: ClaimerEdit, rect: Rect): ClaimerNewRectEdit {
  const polygonsToMerge: Polygon[] = [];
...
export function completeNewRectEditAction(curr: ClaimerNewRectEdit): ClaimerEdit {
  return {
    type: ACTION_TYPES.CLAIMER_EDIT,
    plotId: curr.plotId,
    polygon: curr.polygon,
...
export function updateResizeAction(
  ...
  if (prevOrigX === originalX) {
    ...
    vertices[prevIndex] = [prevOrigX + deltaX, prevOrigY] as (typeof vertices)[number];
  } else if (prevOrigY === originalY) {
    // Horizontal neighbor (shares Y) - apply deltaY only
    vertices[prevIndex] = [prevOrigX, prevOrigY + deltaY] as (typeof vertices)[number];
  }
  ...
  if (nextOrigX === originalX) {
    // Vertical neighbor (shares X) - apply deltaX only
    vertices[nextIndex] = [nextOrigX + deltaX, nextOrigY] as (typeof vertices)[number];
  } else if (nextOrigY === originalY) {
    // Horizontal neighbor (shares Y) - apply deltaY only
    vertices[nextIndex] = [nextOrigX, nextOrigY + deltaY] as (typeof vertices)[number];
  }
...
function onPointerDown(
  ...
  if (context.activeAction?.type === ACTION_TYPES.CLAIMER_EDIT) {
    ...
    const clickedVertexIndex = context.activeAction.polygon.vertices.findIndex(
      ([vx, vy]) =>
  ...
  if (context.activeAction?.type === ACTION_TYPES.CLAIMER_CREATE) {
    ...
    if (currentPolygon != null) {
      ...
      if (clickedVertexIndex !== -1) {
        const updatedContext = {
          ...context,
          activeAction: startResizeCreateAction(clickedVertexIndex, currentPolygon),
        };
        updateCursor(updatedContext);
        return updatedContext;
      }
...
function onPointerOut(
  ...
  if (context.activeAction?.type === ACTION_TYPES.CLAIMER_NEW_RECT_CREATE) {
    const completedRectAction = completeNewRectCreateAction(context.activeAction);
  ...
  if (context.activeAction?.type === ACTION_TYPES.CLAIMER_NEW_RECT_EDIT) {
    const completedRectAction = completeNewRectEditAction(context.activeAction);
