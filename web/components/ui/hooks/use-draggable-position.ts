...
export function useDraggablePosition({
  ...
  const setPosition = useCallback(
    (newPosition: { x: number; y: number }) => {
      ...
      if (constrainToViewport) {
        ...
        if (elementRect == null) {
          throw new Error("Element ref in setPosition of useDraggablePosition");
        }
        constrainedPosition = {
          x: Math.max(
            10,
            Math.min(window.innerWidth - elementRect.width - VIEWPORT_MARGIN, newPosition.x),
          ),
          y: Math.max(
            10,
            Math.min(window.innerHeight - elementRect.height - VIEWPORT_MARGIN, newPosition.y),
          ),
        };
      }
  ...
  const onPointerDown = useCallback(
    (event: React.PointerEvent) => {
      ...
      if (elementRect == null) {
        throw new Error("Element ref in handleMouseDown of useDraggablePosition");
      }
