...
export function useCornerAnchor({
  ...
  const calculateClosestCorner = useCallback(
    (pos: { x: number; y: number }, windowWidth: number, windowHeight: number): Corner => {
  ...
  useEffect(() => {
    ...
    const closestCorner = calculateClosestCorner(position, windowWidth, windowHeight);
    ...
    setAnchoredPosition(position);
  }, [isEnabled, position, elementRef, calculateClosestCorner, calculateOffsetFromCorner]);
  ...
  useEffect(() => {
    ...
    const handleResize = () => {
      ...
      const constrainedPosition = {
        x: Math.max(10, Math.min(newWindowWidth - elementRect.width, newPosition.x)),
        y: Math.max(0, Math.min(newWindowHeight - elementRect.height, newPosition.y)),
      };
  ...
  }, [isEnabled, currentCorner, cornerOffset, elementRef, getPositionFromCornerOffset]);
