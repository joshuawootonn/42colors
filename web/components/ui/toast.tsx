...
export function throttleByKey<T extends Omit<ToastProps, "id">>(callback: (p: T) => void) {
...
export const TOASTS = {
  ...
  loginToSavePixels: (buttonOptions: { label: string; onClick: () => void }) => ({
  ...
  cannotBucketOtherPlot: {
    ...
    description: "It's someone else's plot. Claim a plot for yourself and try again.",
  },
...
export const toasts = {
  ...
  cantMoveToDeletedPlot: (deletedAt: string) => toast(TOASTS.cantMoveToDeletedPlot(deletedAt)),
  ...
  return (
    ...
    >
      ...
      <Collapsible.Root
        ...
        className={cn("flex-grow border-1.5 border-primary bg-secondary", "py-4 pl-2.5 pr-3")}
      >
        <Collapsible.Trigger
          className={cn(
        ...
        >
          <motion.svg
            className={cn("flex items-center justify-center text-primary", "size-5")}
            width={24}
            height={24}
            viewBox="0 0 24 24"
            fill="none"
        ...
        >
          <p className={cn("text-pretty pl-2 text-sm text-gray-500")}>{description}</p>
          {button && (
