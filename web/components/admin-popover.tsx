        ...
        <div className="space-y-2 py-2">
          <div className="flex items-center justify-between px-2">
            ...
            <Switch
              ...
              onCheckedChange={() => store.trigger.toggleAdminCanvasEditing()}
            />
          </div>
          {isAdminCanvasEditingEnabled && (
            <p className="px-2 text-xs text-amber-600">Warning: You can draw on any plot</p>
          )}
          <div className="border-t-1.5 border-border"></div>
          <div className="flex items-center justify-between px-2">
            <label htmlFor="admin-edit-all" className="text-sm font-medium">
              Enable admin plot editing
            </label>
            <Switch
              id="admin-edit-all"
              checked={isAdminPlotEditingEnabled ?? false}
              onCheckedChange={() => store.trigger.toggleAdminPlotEditing()}
            />
          </div>
          {isAdminPlotEditingEnabled && (
            <p className="px-2 text-xs text-amber-600">Warning: You can edit/delete any plot</p>
          )}
          <div className="border-t-1.5 border-border"></div>
          <div className="flex items-center justify-between px-2">
            <label htmlFor="admin-zoom" className="text-sm font-medium">
              Enable admin zoom levels
            </label>
            <Switch
              id="admin-zoom"
              checked={isAdminZoomEnabled ?? false}
              onCheckedChange={() => store.trigger.toggleAdminZoom()}
            />
          </div>
          {isAdminZoomEnabled && (
            <p className="px-2 text-xs text-muted-foreground">Extended zoom range: 10-800</p>
          )}
          ...
          {isSearching && <div className="px-2 text-sm text-muted-foreground">Searching...</div>}
          ...
          {searchResults && searchResults.length === 0 && (
            <div className="px-2 text-sm text-muted-foreground">No users found</div>
          )}

          {selectedUser && (
            <div className="space-y-2 border-t px-2 pt-4">
              <div className="text-sm">
                <div className="font-medium">Selected: {selectedUser.email}</div>
                <div className="text-muted-foreground">
                  Current balance: {selectedUser.balance} pixels
                </div>
              </div>
              ...
              <Button
                onClick={handleGrant}
                disabled={!grantAmount || parseInt(grantAmount, 10) <= 0 || grantMutation.isPending}
                className="w-full"
              >
                {grantMutation.isPending ? "Granting..." : "Grant Pixels"}
              </Button>
              ...
              {grantMutation.isSuccess && (
                <div className="text-sm text-green-600">{grantMutation.data.message}</div>
              )}
            </div>
          )}
        </div>
